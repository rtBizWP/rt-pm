...
if( !class_exists( 'Rt_PM_Project' ) ) {
	class Rt_PM_Project {
		...
		function register_custom_pages() {
			...
			$author_cap = ( function_exists( 'rt_biz_get_access_role_cap' ) ) ? rt_biz_get_access_role_cap( RT_CRM_TEXT_DOMAIN, 'author' ) : '';
		...
		function top_accounts() {
			global $rt_crm_dashboard, $wpdb;
			$table_name = rtcrm_get_lead_table_name();
			$account = ( function_exists( 'rt_biz_get_organization_post_type' ) ) ? rt_biz_get_organization_post_type() : '';
		...
		function top_clients() {
			global $rt_crm_dashboard, $wpdb;
			$table_name = rtcrm_get_lead_table_name();
			$contact = ( function_exists( 'rt_biz_get_person_post_type' ) ) ? rt_biz_get_person_post_type() : '';
			$account = ( function_exists( 'rt_biz_get_organization_post_type' ) ) ? rt_biz_get_organization_post_type() : '';
		...
		function ui() {
			...
			rtcrm_get_template('admin/add-module.php', array( 'rt_attributes_model' => $rt_attributes_model ) );
		}
