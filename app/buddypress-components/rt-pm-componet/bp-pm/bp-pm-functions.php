...
/**
 * Project detail screen action panel buttons
 * @param $post_id
 */
function render_project_summary_buttons( $post_id ) {
	...
	if ( isset( $post_id ) ) {
		...
		if ( isset( $get_post_status ) && $get_post_status == 'trash' ) {
			...
			$archive_action    = 'untrash';
			$archive_button    = __( 'Unarchive' );
			$button_archive_id = 'button-unarchive';
		} else {

			$archive_action    = 'trash';
			$archive_button    = __( 'Archive' );
			$button_archive_id = 'button-archive';
		}

		$url_query_parameter = array(
			'action'        => $archive_action,
			'rt_project_id' => $post_id
		);

		$action_url = add_query_arg( $url_query_parameter, $rt_pm_bp_pm->get_component_root_url() );
		$trash_untrash_url = wp_nonce_url( $action_url, 'rtpm_project_trash_action' );

		$url_query_parameter['action'] = 'delete';
		$action_url = add_query_arg( $url_query_parameter, $rt_pm_bp_pm->get_component_root_url() );
		$delete_url = wp_nonce_url( $action_url, 'rtpm_project_trash_action' );
		?>
		<button id="top-<?php echo $button_archive_id; ?>" class="mybutton" data-href="<?php echo $trash_untrash_url ?>" class=""><?php _e( $archive_button ); ?></button>
		<button id="top-button-trash" class="mybutton" data-href="<?php echo $delete_url ?>" class=""><?php _e( 'Delete' ); ?></button>
	<?php }
}
...
function rtpm_bp_get_project_details_url( $project_id ) {
	...
	$project_edit_link = add_query_arg( array(
		'rt_project_id' => $project_id,
		'action'        => 'edit',
		'post_type'     => $rt_pm_project->post_type,
		'tab'           => "{$rt_pm_project->post_type}-details"
